...
class AnsibleRoleContractTests(unittest.TestCase):
    ...
    def test_api_keeps_directory_inode_and_uses_reviewed_release(self):
        ...
        self.assertIn("ReadWritePaths={{ farm_api_auth_directory.stdout", unit)
        self.assertIn("Validate the exact writable authentication directory", tasks)
        self.assertIn("parent.resolve(strict=True) == parent", tasks)
        self.assertIn("info.st_uid in (0, 9999)", tasks)
